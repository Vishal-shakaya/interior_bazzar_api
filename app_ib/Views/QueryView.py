...
from app_ib.Controllers.Query.QueryController import QUERY_CONTROLLER
...
async def CreateQueryView(request):
    try:
        ...
        auth_resp = await  asyncio.gather(QUERY_CONTROLLER.CrateQuery(user_ins=user_ins, data=data,buss_ins=buss_ins))
...
async def UpdateQueryByIdView(request,id):
    try:
        return JsonResponse({"result": 'success'})
    except Exception as e:
        print(f'{e}')
